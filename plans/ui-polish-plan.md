# UI Polish Plan — Post-Anvil Migration

## Completed (this session)
- [x] Committed anvil migration (175 files, radix -> @base-ui/react)
...
2. **Two Button APIs coexist** — `@/components/ui/button.tsx` (radix-based, uses `variant/size/tone`) and `@/anvil/button.tsx` (base-ui, uses `variant/intent/size`). Components import from both inconsistently. Need to converge on one API.

3. **Dialog systems coexist** — `@/components/ui/dialog.tsx` (radix) and `@/anvil/modal.tsx` (base-ui). Some dialogs use one, some the other. Need to converge.
...
8. **Input fields** — Verify all number inputs (size, price, leverage) have consistent height, border radius, and focus states using the anvil design tokens.

9. **Toast/notification styling** — Verify order toasts use anvil AlertGlobal component with proper tone colors.

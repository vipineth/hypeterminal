			...
			<div className={cn(alertVariants({ tone, className }))} ref={ref} role="alert" {...props}>
				...
				>
					...
					<div className="flex flex-1 flex-col gap-1 min-w-px">
						{title && (
							<p className={cn("font-semibold text-balance", isLarge ? "text-base" : "text-xs", colors.title)}>
								{title}
							</p>
						)}
						{description && (
							<p className={cn("font-normal text-pretty", isLarge ? "text-sm" : "text-xs", colors.description)}>
								{description}
							</p>
						)}
						{children}

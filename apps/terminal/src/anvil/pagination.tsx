...
const pageButtonVariants = cva(
	[
		"inline-flex items-center justify-center size-10 rounded-8",
...
const navButtonVariants = cva(
	...
	],
	{
		variants: {
			variant: {
				desktop: "gap-2 h-10 text-xs font-normal text-text-weak",
				mobile: "h-10 text-icon-neutral",
			},
		},
		defaultVariants: {
			variant: "desktop",
...
const Pagination = React.forwardRef<HTMLElement, PaginationProps>(
	(
		{ className, currentPage, totalPages, onPageChange, totalItems, itemsPerPage = 10, variant = "desktop", ...props },
		ref,
	) => {
			...
			return (
				...
				>
					<button
						...
						className={cn(navButtonVariants({ variant: "mobile" }))}
					>
			...
			<nav ref={ref} aria-label="Pagination" className={cn("flex items-center justify-between", className)} {...props}>
				<div className="flex items-center gap-2">
					<button
						...
						data-disabled={isFirstPage ? "" : undefined}
						aria-label="Previous page"
						className={cn(navButtonVariants({ variant: "desktop" }), "pr-4")}
					>
						<ArrowLeft size={20} className="shrink-0 text-icon-neutral" />
						Previous
					</button>
					...
					{pages.map((page, index) =>
						page === "ellipsis" ? (
							<span
								key={`ellipsis-${index}`}
								className="inline-flex items-center justify-center size-10 text-xs font-normal text-text-weak select-none"
							>
								&hellip;
							</span>
						) : (
							<button
								...
								className={cn(pageButtonVariants({ current: page === currentPage }))}
							>
								{page}
							</button>
						),
					...
					<button
						...
						data-disabled={isLastPage ? "" : undefined}
						aria-label="Next page"
						className={cn(navButtonVariants({ variant: "desktop" }), "pl-4")}
					>
						Next
						<ArrowRight size={20} className="shrink-0 text-icon-neutral" />
					</button>
				</div>

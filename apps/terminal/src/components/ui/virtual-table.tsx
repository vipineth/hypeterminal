...
export function VirtualTable<TData extends RowData>({
	...
	return (
		<div className={cn("relative overflow-hidden", className)}>
			<div className="sticky top-0 z-10 bg-bg-overlay border-b border-stroke-weak/40">
					...
					<div key={headerGroup.id} className="flex w-full">
						{headerGroup.headers.map((header) => {
							...
							const headerClassName = cn(
								"flex items-center px-3 py-2 text-xs font-medium text-text-weak uppercase tracking-wider",
								canSort && "cursor-pointer select-none hover:text-text-strong transition-colors",
							);
					...
					{virtualItems.map((virtualItem) => {
						...
						return (
							<div
								...
								className="absolute top-0 left-0 w-full flex items-center border-b border-stroke-weak/20 hover:bg-bg-raised/30 transition-colors"
								style={{

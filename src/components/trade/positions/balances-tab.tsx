	...
	function renderBalanceRow(row: BalanceRow, index: number) {
		...
		return (
			...
			>
				...
				<TableCell className="text-3xs text-right tabular-nums text-market-up-primary py-1.5">
					{formatUSD(row.usdValue, { compact: true })}
				</TableCell>
				<TableCell className="text-3xs text-right tabular-nums py-1.5">
					{pnlData ? (
						<span className={pnlData.pnl >= 0 ? "text-market-up-primary" : "text-market-down-primary"}>
							{pnlData.pnl >= 0 ? "+" : ""}
							{formatUSD(pnlData.pnl, { compact: true })} ({pnlData.pnlPercent >= 0 ? "+" : ""}
							{pnlData.pnlPercent.toFixed(1)}%)
						</span>
					) : (
						<span className="text-fg-700">—</span>
					)}
				</TableCell>
					...
					<div className="flex items-center justify-end gap-1">
						{canTransfer && (
							<Button
								...
								className="text-4xs text-status-info hover:text-status-info/80 hover:bg-transparent px-1.5 py-0.5 gap-1"
							>
	...
	return (
		<div className="flex-1 min-h-0 flex flex-col p-2">
			<div className="text-3xs uppercase tracking-wider text-fg-700 mb-1.5 flex items-center gap-2">
				...
				<span className="text-market-up-primary tabular-nums">
						...
						<Table className="w-auto min-w-full">
							<TableHeader>
								<TableRow className="border-border/40 bg-surface-300 hover:bg-surface-300">
									...
									<TableHead className="text-4xs font-medium uppercase tracking-wider text-fg-500 text-right h-7 w-22.5">
										{t`Available`}
									</TableHead>
									<TableHead className="text-4xs font-medium uppercase tracking-wider text-fg-500 text-right h-7 w-20">
										{t`In Use`}
									</TableHead>
									<TableHead className="text-4xs font-medium uppercase tracking-wider text-fg-500 text-right h-7 w-22.5">
										{t`Total`}
									...
									<TableHead className="text-4xs font-medium uppercase tracking-wider text-fg-500 text-right h-7 w-25">
										{t`PNL`}
									</TableHead>
									<TableHead className="text-4xs font-medium uppercase tracking-wider text-fg-500 text-right h-7 w-20">
										{t`Actions`}
								...
								{perpBalances.length > 0 && (
									<>
										<TableRow className="border-border/40 hover:bg-transparent">
											<TableCell
												colSpan={7}
												className="text-3xs uppercase tracking-wider text-action-primary/80 bg-action-primary/5 py-1 font-medium"
											>
												{t`Perpetuals`}
											</TableCell>
										</TableRow>
								...
								{spotBalancesFiltered.length > 0 && (
									<>
										<TableRow className="border-border/40 hover:bg-transparent">
											<TableCell
												colSpan={7}
												className="text-3xs uppercase tracking-wider text-status-warning/80 bg-status-warning/5 py-1 font-medium"
											>
												{t`Spot`}
											</TableCell>
										</TableRow>
